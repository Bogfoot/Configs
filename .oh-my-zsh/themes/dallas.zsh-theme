...
# DALLAS_CURRENT_TIME_="祥%{$fg[white]%}{%{$fg[yellow]%}%D %T%{$fg[white]%}}%{$reset_color%}"
DALLAS_CURRENT_TIME_="祥%{$fg[green]%}%D %T%{$reset_color%} "
# Grab the current machine name: muscato
# DALLAS_CURRENT_MACH_="%{$fg[green]%}%m%{$fg[white]%}:%{$reset_color%}"
DALLAS_CURRENT_MACH_=""
...
DALLAS_PROMPT_CHAR_="%{$fg[white]%}%(!.#.ﬦ)%{$reset_color%}"
...
# Put it all together!
PROMPT="$DALLAS_CURRENT_TIME_\$(ruby_prompt_info)$DALLAS_CURRENT_MACH_$DALLAS_CURRENT_LOCA_${NEWLINE}  $DALLAS_CURRENT_USER_ $DALLAS_PROMPT_CHAR_ "
